...
- `family_source_signals` — one row per `(family_id, source)` with at least one membership. `resource_count` and metric availability counts are explicit. `demand_percentile_p50/p75/p90/p95` and `ge90/ge95` counts/shares summarize YEE-29's source-local percentile only. `freshness_age_days` p50/p75/p90 plus le30/le90/gt365 counts/shares and `age_days_p50` retain their YEE-29 meanings. `downloads_total` p50/p75/p90 remain source-native and are never summed across sources. The concentration fields are named `demand_concentration_top1_share`, `demand_concentration_top3_share`, and `demand_concentration_hhi`; they use non-null downloads within only that family/source and are null if the usable total is not positive.
...
Voxel `paid_state` is summarized in the normalized Voxel source row as `free_count`, `paid_count`, and `unknown_paid_state_count`; `paid_known_count` is free+paid, and `paid_share_known` is paid divided by known free+paid resources, or null when there are none. The wide `family_features` fields use the source prefix (for example `voxel_free_count` and `voxel_demand_concentration_hhi`). Non-Voxel paid state is not evidence of free or paid supply.
